...
```
时间复杂度 $O(n)$，当 $n$ 特别大时会超时。  

快速幂是对暴力算法的优化，可以在 $O(log(n))$ 时间内算出。  

### 思路

核心观察：指数可以拆分  

利用幂的运算性质：$a^{m+n} = a^m \times a^n$
...
注意这里 $8、4、1$ 正好是 $n$ 的二进制中为 $1$ 的那些位对应的权值。  

如何得到 $a^1, a^2, a^4, a^8, \dots $ ？

不需要单独计算。利用自身不断平方：  

$a^1 \xrightarrow{\times a} a^2 \xrightarrow{\times a^2} a^4 \xrightarrow{\times a^4} a^8 \cdots$
...
- 如果 `n & 1 == 1`，说明这一位是 1，把当前的 $a^{2^k}$ 乘进答案。
- 如果 `n & 1 == 0`，说明这一位是 0，跳过。
