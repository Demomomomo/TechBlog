<template><div><h2 id="跳石头-二分" tabindex="-1"><a class="header-anchor" href="#跳石头-二分" aria-hidden="true">#</a> 跳石头（二分）</h2>
<p>题目描述</p>
...
4</pre>
		...
		</tr></tbody></table><p>数据范围与提示</p>
<p>对于 20\%20% 的数据，0 \leq M \leq N \leq 100≤M≤N≤10。<br />
对于 50\%50% 的数据，0 \leq M \leq N \leq 1000≤M≤N≤100。<br />
...
<p>著名出题人小Q出过非常多的题目，在这个漫长的过程中他发现，确定题目的数据范围是非常痛苦的一件事。<br /><br />
每当思考完一道题目的时间效率，小Q就需要结合时限以及评测机配置来设置合理的数据范围。<br /><br />
因为确定数据范围是一件痛苦的事，小Q出了非常多的题目之后，都没有它们设置数据范围。对于一道题目，小Q会告诉你他的算法的时间复杂度为O(n^a\log^bn)O(nalogbn)，且蕴含在这个大OO记号下的常数为11。同时，小Q还会告诉你评测机在规定时限内可以执行kk条指令。小Q认为只要n^a\left(\lceil\log_2n\rceil\right)^bna(⌈log2​n⌉)b不超过kk，那么就是合理的数据范围。其中，\lceil x\rceil⌈x⌉表示最小的不小于xx的正整数，即xx上取整。<br /><br />
自然，小Q希望题目的数据范围nn越大越好，他希望你写一个程序帮助他设置最大的数据范围。</p>
<p>Input</p>
<p>第一行包含一个正整数T(1\leq T\leq 1000)T(1≤T≤1000)，表示测试数据的组数。<br /><br />
每组数据包含一行三个正整数a,b,k(1\leq a,b\leq 10,10^6\leq k\leq 10^{18})a,b,k(1≤a,b≤10,106≤k≤1018)，分别描述时间复杂度以及允许的指令数。</p>
<p>Output</p>
...
<p>3 1 1 100000000 2 1 100000000 1 3 200000000</pre>
</td>
<td>
<pre></p>
</pre>
...
<p>4347826 2886 48828</pre>
</td>
</tr></tbody></table><p> 思路：二分n</p></p>
<p>注意log2不能使用longlong，需要手写log2</p>
<pre>
<code class="hljs">ll log(ll x){
	ll con=0;
	ll ans=1;
	while(ans&lt;x){
		con++;
...
<pre>
<code class="hljs">bool cheek(ll n){
	__int128 ans=1;
	for(int i=0;i&lt;a;i++){
		ans*=n;
...
<h3>输入格式</h3>
<p>第一行输入两个整数 nn 和 mm，分别表示数组的长度和查询的次数。</p>
<p>接下来一行有 nn 个整数 a_iai​。</p>
<p>接下来 mm 行，每行有 11 个整数 xx，表示蒜头君询问的整数。</p>
<h3>输出格式</h3>
<p>对于每次查询，如果可以找到，输出<code>"YES"</code>，否则输出<code>"NO"</code>。</p>
<h3>数据范围</h3>
<p>1 \le n, m \le 10^5, 0 \le x \le 10^61≤n,m≤105,0≤x≤106。</p>
<p>Sample 1</p>
<table><thead><tr><th>Inputcopy</th>
			<th>Outputcopy</th>
		</tr></thead><tbody><tr><td>
