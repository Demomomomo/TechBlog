<template><div><h2 id="最短编辑距离" tabindex="-1"><a class="header-anchor" href="#最短编辑距离" aria-hidden="true">#</a> 最短编辑距离</h2>
<p>原题链接：<a href="https://www.acwing.com/problem/content/904/" target="_blank" rel="noopener noreferrer">https://www.acwing.com/problem/content/904/<ExternalLinkIcon/></a><br>
题意：<br>
给定两个字符串A和B，现在要将A经过若干操作变为B，可进行的操作有：<br>
1.删除–将字符串A中的某个字符删除。<br>
2.插入–在字符串A的某个位置插入某个字符。<br>
3.替换–将字符串A中的某个字符替换为另一个字符。</p>
<p>求将A变为b至少需要多少次操作。</p>
<p>思路：<br>
f[i][j]表示A[1<sub>i]变为B[1</sub>j]需要的最小操作次数。<br>
因为求最小值，先将f初始化为极大值。<br>
因为当ab字符串都是空的时候，不用进行操作也一样，那么f[0][0]=0.<br>
将a[1~i]变为b[0]的时候，我们需要将a全部删除，那么f[i][0]=i<br>
将a[0]变为b[1~i]的时候，我们需要将a加上i个字符，那么f[0][i]=i<br>
对于第一个删除操作，当a[1<sub>i-1]已经变为b[1</sub>j]的时候，我们需要将a的i删除，那么f[i][j]=min(f[i][j],f[i-1][j]+1)<br>
对于第二个插入操作，当a[1<sub>i]已经变为b[1</sub>j-1]，那么我们需要再a的最后加上一个字符，那么f[i][j]=min(f[i][j],f[i][j-1]+1)<br>
对于第三个替换操作，如果a[i]!=b[j]，那么就需要替换，那么f[i][j]=min(f[i][j],f[i-1][j-1]+1)<br>
那么当a[i]==b[j]的时候，我们不用操作，f[i][j]=min(f[i][j],f[i-1][j-1]+1)</p>
