...
一个割点至少属于两个联通分量<br>
极大的不包含割点的联通块，就被称为点双联通分量</p>
<h2 id="tarjan算法求边双联通分量" tabindex="-1"><a class="header-anchor" href="#tarjan算法求边双联通分量" aria-hidden="true">#</a> tarjan算法求边双联通分量</h2>
<p>思路：
dfn[x]表示最早走到x的时间<br>
low[x]表示从x开始走，能走到的点的最小时间<br>
判断一个边是不是桥：x-y之间有一条边，（x比y先遍历到）如果y结点能走到x的祖宗结点，他就不是桥，如果不能说明他是桥<br>
所以如果当xy之间的边是桥的话，dfn[x] &lt; low[y]<br>
如何找到一个边双联通分量：<br>
方法1：将所有的桥删掉，剩下的每一个联通块就是边双联通分量<br>
方法2：用一个栈来存遍历到的点，遍历完x之后，如果dfn[x] == low[x]，说明从x走不到让他的父结点，那么x与他的父节点之间的边 就是一个桥，x的子树就是x所在的边双联通分量</p>
<p>方法2代码：</p>
